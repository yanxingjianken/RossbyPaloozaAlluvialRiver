...
| `06_anim_per_wavelength.py` | **one 4-panel movie per seeded wavelength** — each of the 15 fig04 components (k*=0.1…1.5) run alone on the same Lx=20π reach → `figures/per_wavelength/planform_k*.mp4` (+previews), plus `fig08` contact sheet. Each frame is 2×2: **ψ_total** (streamlines of the meandering jet) · **ψ'** (perturbation) · **u'v'** (momentum flux) · **stats** (σ measured/EVP, crests c*, momentum c_g, gain, + a live log\|a(t)\| erosion-growth curve). Subtitle carries λ/2b, σ*, c*_phase AND c*_group. The growth curve makes erosion explicit; the pattern's translation is the phase speed c* (NOT mean-flow advection — the bank eq has no ∂x term). Flags: `--kstars`, `--friction momentum`, `--frames N`. |
| `derivations/ns_to_meander.tex` (+`.pdf`) | **15-slide Beamer deck**: the full NS→both-models derivation — the shared depth-average, the Station-2 fork (Ikeda intrinsic-coords vs our curl), Fr scaling, Eq.(7) assembly, the 3 curl-born extra terms, the 3-level closure (friction interior / erosion boundary), the Dedalus 5-field/5-eq solver + 1e-10 verification, the group-velocity sign flip, and the advection-vs-erosion clarification. Compile: `lualatex ns_to_meander.tex` (×2). |
