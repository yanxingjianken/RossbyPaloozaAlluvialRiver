...
| [`dedalus_meander2/`](dedalus_meander2/) | **variable-depth PV generalization** (deck-p.8 erodible bottom, rung 2) | drops the flat bed → PV conservation D/Dt(ζ/H) + topographic-shear β=∂y(ζ̄/H̄); single-driver architecture (one Dedalus file → HDF5; all viz in postprocessing/). ζ-auxiliary + Ĥ²-weighted well-banded form. Verified: flat-bed reduction **0.0**, d3 EVP vs FD-GEP **4e−9** (flat+bumped, both closures), IVP vs EVP **0.1%**; deeper thalweg shifts the group-velocity flip k_g. Honest: non-rotating ⇒ bed modulates shear-β (D=0→no β; f0 knob for true topo-Rossby); x-varying bed deferred |
| [`dedalus_rigid_lid_to_share/`](dedalus_rigid_lid_to_share/) | **single-file rigid-lid model, for sharing** | one self-contained `rigid_lid_meander.py`: edit the CONFIG at the top, run, get an erodible-bank IVP. Supports a general **H(x,y)** bed; prognostic q=ζ/H with a discharge-conserving u₀=H̄ū/H; no verification ladder, no CLI — straight into the IVP. Meant for other Dedalus users to modify, not for this repo's results (demo: ×58 bank growth) |
| [`dedalus_meander_full_SW/`](dedalus_meander_full_SW/) | **full shallow-water, curvilinear (s,n)** (what wave is the meander?) | keeps the **free surface η** (mass eqn has η_t) ⇒ gravity waves AND the vortical wave coexist; flow **divergent** (no streamfunction — solve (u_s,u_n,η), diagnose PV). Finite-amplitude meander in channel-fitted (s,n), σ=1+nC̄; parabolic jet (const channel-β), superelevation; erodible **centerline** via antisymmetric near-bank velocity (Ikeda); IVP-only. Perturbation is **broadband** (all k at once ⇒ one run = the whole dispersion relation), so runs are keyed to physics `H·bank·C_f·U₀·Δ`, never to a seeded wavelength. **Result (headline RETRACTED 2026-07-21):** the "meander = the vortical/Rossby wave" claim does *not* survive its own diagnostics. 11 runs at Ns=128 (σ(k) converged vs Ns=64 to **0.0–0.1%**): **every growing run has `T_shear ≤ 0`, and the one run with `T_shear > 0` decays** — when it grows it is *losing* energy to the mean flow (quote the sign only; the magnitude is resolution-sensitive, −3.16→−0.36 from Ns 64→128). A smooth single-signed ∂ₙq̄ admits no discrete vortical mode anyway (Case 1960), and the Froude test is analytically null (F enters only as F² on continuity). **β is not necessary but not irrelevant:** at *matched bank speed* U₀=0.4, σ(k=1.8) = +0.165 (Δ=0.6) → +0.060 (Δ=0, β removed, still unstable) → −0.053 (Δ=−0.3, **stable**) — 2.75× weaker without β, not faster (an earlier claim of "grows fastest" was a U₀ confound, now controlled). **Wavelength selection exists but by the wrong mechanism:** the k² damping is ~26× ν and shear-set, **k_peak·Δ ≈ 1.10** (±3% across C_f ×10, bed, bank 0→0.15). Honest reframing: a **forced morphodynamic boundary mode** — balanced (‖δ'‖/‖ζ'‖ ≈ 0.01–0.05), bank-driven (σ∝E), shear-shaped. Corollary: the free surface is passive ⇒ the rigid-lid packages above are **validated for F≲0.9**. Outputs: dispersion figure + one absolute-Eulerian momentum-flux movie per configuration |
