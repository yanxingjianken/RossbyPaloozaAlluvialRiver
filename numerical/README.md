...
| [`dedalus_meander_full_SW/`](dedalus_meander_full_SW/) | **full shallow-water, curvilinear (s,n)** (what wave is the meander?) | keeps the **free surface η** (mass eqn has η_t) ⇒ gravity waves AND the vortical wave coexist; flow **divergent** (no streamfunction — solve (u_s,u_n,η), diagnose PV). Finite-amplitude meander in channel-fitted (s,n), σ=1+nC̄; parabolic jet (const channel-β), superelevation; erodible **centerline** via antisymmetric near-bank velocity (Ikeda); IVP-only. Perturbation is **broadband** (all k at once ⇒ one run = the whole dispersion relation), so runs are keyed to physics `H·bank·C_f·U₀·Δ`, never to a seeded wavelength. **Result (headline RETRACTED 2026-07-21):** the earlier "meander = the vortical/Rossby wave" claim does *not* survive its own diagnostics — `T_shear<0` in all 9 configurations (mean flow is an energy **sink**, not the source), the meander still grows with the channel-β removed entirely (`Δ=0` plug flow grows *fastest*), and a smooth single-signed ∂ₙq̄ admits no discrete vortical mode (Case 1960). The Froude test is analytically null by construction (F enters only as F² on continuity). Honest reframing: a **forced morphodynamic boundary mode** — balanced, but bank-driven (σ∝E), with no wavelength selection (bed frozen ⇒ no Exner ⇒ σ rises to the ν cutoff). Useful corollary: the free surface is passive ⇒ the rigid-lid packages above are **validated for F≲0.9**. Outputs: dispersion figure + one absolute-Eulerian momentum-flux movie per configuration |
