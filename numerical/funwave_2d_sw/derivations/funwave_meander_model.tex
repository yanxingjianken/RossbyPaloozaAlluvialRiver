...
\code{make\_figs.py}, so this document cannot drift from the code. How the secondary flow is
parameterised is \S\ref{sec:secondary}; the full list of approximations that bounds every
result is \S\ref{sec:approx}.
\end{abstract}
...
\begin{enumerate}\itemsep1pt
  ...
  \item \textbf{Depth-averaged}: no vertical coordinate, so secondary (helical) circulation
        cannot be \emph{resolved}. It is instead \emph{parameterised} through the two Ikeda
        closures of \S\ref{sec:secondary} --- drag modulation \eqref{eq:cdeff} and bedload
        deflection \eqref{eq:secondary} --- rather than being absent ($A=2.89$, not $A=0$).
  ...
  \item \textbf{Bedload direction} $=$ depth-averaged flow direction, \emph{plus} the
        secondary-flow deflection \eqref{eq:secondary} toward the inner bank and the
        gravitational return down the transverse slope \eqref{eq:slopedefl}; their balance is
        Ikeda's equilibrium tilt (\S\ref{sec:secondary}).
...
\begin{enumerate}\setcounter{enumi}{10}\itemsep1pt
  \item \textbf{Morphological acceleration}, $\mathrm{MF}=5$ in \eqref{eq:mf}. The bed
        is advanced five times faster than the flow that drives it. Formally this
        ...
        MF-convergence sweep}, not assumed. ($\mathrm{MF}=50$ blew up on the flow-driven
        inner-scour hole before the closures of \S\ref{sec:secondary} redirected it.)
...
of \S\ref{sec:secondary}: with $A=0$ (stock FUNWAVE) and no slope deflection there is no free
bar mode to find, so \eqref{eq:slopedefl}--\eqref{eq:secondary} are a prerequisite --- now
implemented --- not an optional refinement.

% =====================================================================
\section{The secondary-flow closures (gap 1)}
\label{sec:secondary}

A depth-averaged model has no vertical coordinate, so the helical secondary flow of a bend
cannot arise on its own. Its two morphological effects are therefore \emph{parameterised}
following Ikeda, Parker \& Sawai (1981), and both are now active (stock FUNWAVE is the $A=0$
``incised'' limit; the alluvial value is $A=2.89$, Suga 1963). Together they are what turns
the free-vortex \emph{inner}-bank scour of a bare depth-averaged model into the outer-bank
scour and inner point bar of a real alluvial river.

\paragraph{Momentum half (Ikeda eq.~3b), \code{run\_meander.py}, \code{FRICTION\_MATRIX}.}
The tangential-momentum balance
$U\ds u' = -g\ds\xi' - (C_fU^2/H)(2u'/U-\xi'/H+\eta'/H)$
carries the secondary flow only through the last friction term, closed by the equilibrium
bed tilt $\eta'/H=-A\,\mathcal C\,n$. FUNWAVE already solves the other two terms (its
$-C_d u|u|$ \emph{is} $2u'/U$; its $-g\,\ds\eta$ \emph{is} $-\xi'/H$), so only $\eta'/H$ is
added --- as a spatial modulation of the drag,
\begin{equation}
  C_d^{\rm eff}(x,y)=C_d\bigl(1+A\,\kappa\,n\bigr),
  \label{eq:cdeff}
\end{equation}
in the code's $(\kappa,n)$ sign convention (outer bank $\kappa n<0$, verified against the
centre-of-curvature distance in \code{tests/test\_bathy.py}~7b). There $C_d^{\rm eff}<C_d$,
the flow accelerates outward, and the free-vortex inner-fast tendency is opposed. This term
is \emph{weak} at $R/W\!\sim\!3$: the friction-adjustment/wavelength factor
$k_f/\sqrt{k_f^2+k_\lambda^2}\approx0.17$ (with $k_f=2C_f/H$, $k_\lambda=2\pi/\lambda$) caps
the redistribution, so the steady outer/inner apex speed ratio moves only
$0.63\!\to\!0.66$ --- it reduces but does not reverse the free vortex. The bar therefore
needs the bedload half.

\paragraph{Bedload half (Ikeda eq.~6) --- the bar-building term, \code{mod\_sediment.F}.}
The point bar is a \emph{bedload} feature. The secondary flow rotates the near-bed velocity,
hence the bedload vector, toward the inner bank by
\begin{equation}
  \delta=\frac{A\,\kappa\,H}{f(\theta)},\qquad f(\theta)=A_{\rm bl}\sqrt\theta,\quad
  \theta=\frac{\tau_b}{(s-1)gD_{50}},
  \label{eq:secondary}
\end{equation}
added to the flow direction, with $\kappa(x,y)$ read from a curvature field. Balanced against
the transverse bed-slope (Talmon 1995) return flux
\begin{equation}
  q_{b,n}=|\bm q_b|\Bigl[\sin\delta-\tfrac{1}{f(\theta)}\dn z_b\Bigr],
  \label{eq:slopedefl}
\end{equation}
the transverse balance $q_{b,n}=0$ gives $\dn z_b=-A\,\kappa\,H$ --- Ikeda's equilibrium tilt
(deep OUTER, shallow INNER), \emph{independent of} $f(\theta)$, which cancels. Because it is a
forcing (not diffusion), the deflection builds the bar from a flat bed, where the Talmon term
alone would only diffuse toward flat; the Talmon term is retained as the down-slope stabiliser
that arrests the scour hole.

\code{postprocessing/01\_validate.py} reports the measured inner/outer bed-change split, and
\code{04\_xsection.py} shows the transverse profile evolving. The prediction these closures
make is outer scour \emph{with} an inner point bar; the stock ($A=0$) model scours the inner
bank instead.

Still absent: any resolved vertical structure; rotation ($f=0$, so ``Rossby wave'' here is a
shear-$\beta$ vortical wave, never a planetary one); along-channel depth variation in the
initial bed (any $h(x)$ must \emph{emerge} from \eqref{eq:exner}); and width variation as an
